...
# Phase 2: Crol pays herself (a circle route through Alice and Bob) - but do not return secret - attack.
# Use case: Crol does not return secret - only the update fail once her htlc reaches timeout. 
# This script shows the attack using circular route
...
log_file="$HOME/.lightning/logs/exp5_basic_circular_route_attack_$(date +'%d-%m-%Y-%H:%M:%S').log"
