...
# A route from Crol to Alice, back and forth Alice<->Bob 9 times and back to Crol.
...
# Changes in configurations: Bob configures max-concurrent-htlcs=30
# We show that sending more than 18 htlcs fails.

HOME='/home/ayelet'
. $HOME/.lightning/scripts/logger.sh

log_file="$HOME/.lightning/logs/exp12_different_max_htlcs_b_$(date +'%d-%m-%Y-%H:%M:%S').log"
...
inv=$($HOME/lightning/cli/lightning-cli --rpc-file=$HOME/.lightning/Bob/lightning-rpc invoice 2000000000 "Alice_to_Bob-$(date +'%T:%N')" "$(date +'%T:%N') tx of 200000000 msat from Alice to Crol")
...
$HOME/lightning/cli/lightning-cli --rpc-file=$HOME/.lightning/Crol/lightning-rpc listsendpays


#### Phase 3: Bob pays Crol #####

einfo "\n####### Phase 3: Bob pays Crol #######" |& tee -a $log_file
