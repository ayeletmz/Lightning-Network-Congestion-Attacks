...
void htlcs_notify_new_block(struct lightningd *ld, u32 height)
{
	...
	do {
		...
		for (hin = htlc_in_map_first(&ld->htlcs_in, &ini);
			...
			if (!hin->preimage)
				log_debug(ld->log, "hin 2.1 cltv_expiry=%d, htlc_in_deadline=%d", hin->cltv_expiry, htlc_in_deadline(ld, hin));
				if (strcmp((const char*)ld->alias, "Crol") == 0  && height == hin->cltv_expiry) {
					local_fail_htlc(hin, WIRE_PERMANENT_CHANNEL_FAILURE,
				channel->scid);
				}
				continue;
