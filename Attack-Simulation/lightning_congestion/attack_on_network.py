...
class Route:
    ...
    """
    def __init__(self, first_node, last_node, edges, time_lock, capacity):
...
class AttackRoutes:
    ...
    def sort_by_capacity(self):
        sorted_tuples = sorted(zip(self.lengths, self.lock_times,
                                   self.capacities, self.amounts_sent, self.amounts_received,
                                   self.max_htlcs),
                               key=lambda pair: pair[2], reverse=True)
        self.lengths, self.lock_times, self.capacities, self.amounts_sent, self.amounts_received, self.max_htlcs = list(
            zip(*sorted_tuples))

    def sort_by_capacity_and_cost(self):
        # Sorts routes by capacity/amounts_sent in decreasing order
        sorted_tuples = sorted(zip(self.lengths, self.lock_times,
                                   self.capacities, self.amounts_sent, self.amounts_received,
                                   self.max_htlcs),
                               key=lambda pair: pair[2] / pair[3], reverse=True)
        self.lengths, self.lock_times, self.capacities, self.amounts_sent, self.amounts_received, self.max_htlcs = list(
            zip(*sorted_tuples))
    ...
    def remove_expensive_routes(self, cost_upper_bound_msat):
        """
        Returns routes which enable successful payments of amount <= cost_upper_bound_msat
        """
        attacker_results = copy.deepcopy(self)
        filtered = [i for i in list(zip(attacker_results.lengths, attacker_results.lock_times,
                                        attacker_results.capacities, attacker_results.amounts_sent,
                                        attacker_results.amounts_received, attacker_results.max_htlcs))
                    if i[3] <= cost_upper_bound_msat]
        attacker_results.lengths, attacker_results.lock_times, attacker_results.capacities, \
        attacker_results.amounts_sent, attacker_results.amounts_received, attacker_results.max_htlcs = \
            list(zip(*filtered))
        return attacker_results
...
def _append_next_edge_to_route(G, route, lock_period, max_route_length):
    """
    ...
    # tuples of all adjacent channels (adj_node_id, channel_id, capacity, cltv_delta, next_node default cltv_delta)
    adj_channels = [(adj_node_id, channel_id, neighbours[adj_node_id][channel_id]['capacity'],
                     ...
                     CLTV_DELTA_DEFAULTS[G.nodes()[adj_node_id]['implementation']]) for
    ...
    max_capacity_adj_channels = [channel_tup for channel_tup in adj_channels
    ...
    channel_tup = min(max_capacity_adj_channels, key=operator.itemgetter(3))
    ...
    if route.len() < max_route_length:
        return _append_next_edge_to_route(G, route, lock_period, max_route_length)
    route.time_lock -= CLTV_DELTA_DEFAULTS[G.nodes()[route.last_node]['implementation']]
    return route


def _locate_route(G, starting_edge, lock_period, max_route_length):
    """
    ...
    route = Route(first_node, next_node, route_edges, route_time_lock, route_capacity)
...
def _plot_costs(attack_routes):
    ...
    locked_liquidity = np.cumsum(attack_routes.get_capacity_needed_to_attack())
    blockchain_fees = OPEN_CHANNEL_COST_BTC * np.arange(2, 2 * (attack_routes.len() + 1), 2)

    x = np.cumsum(list(map(lambda x: x / 1e8, attack_routes.capacities)))  # 1 BTC = 1e8 SAT
    y = [blockchain_fees, locked_liquidity]
    ...
    plt.ylabel('BTC')
    plt.xlim((-10, 690))
    plt.ylim((-0.008, 0.335))
    plt.savefig("plots/attack_on_network_costs.svg")


def _compute_network_attack_routes(G, lock_period, max_route_length=MAX_ROUTE_LEN):
    """
    Splits G into disjoint routes that can be locked for at least lock_period blocks.
    """
    ...
    logger.info("Combining both subgraphs results into disjoint routes in the network that can be locked for at-least "
...
def attack_on_network(snapshot_path):
    ...
    attack_routes.sort_by_capacity_and_cost()
    _plot_costs(attack_routes.remove_expensive_routes(1000000))
...
def attack_for_different_lock_periods(snapshot_path):
    ...
    x1, x2, y1, y2 = 1351, 1385, 0.923, 0.948
...
def attack_for_different_snapshots(snapshots_dir):
    ...
    for G_str in snapshots_list:
        ...
        x = np.arange(2, 2 * (len(cumulative_attacked_capacity) + 1), 2)
    ...
    for y in attacked_capacity_by_snapshot:
        axins.plot(x, y)
    x1, x2, y1, y2 = 1150, 1170, 0.909, 0.926
    axins.set_xlim(x1, x2)
    axins.set_ylim(y1, y2)
    plt.yticks(visible=False)
    plt.xticks(visible=False)
